...
Das \enword{Keyword} ist die Schlüsselkomponente der Sprache Rayden. In diesem Abschnitt werden die unterschiedlichen Arten und Metatypen erklärt und gezeigt, wofür diese verwendet werden können. Zu Beginn werden die vier Metatypen von \enword{Keywords} erklärt. Die Metatypen bilden die Basis für den Funktionsumfang der Sprache. Ferner werden die unterstützten Arten beschrieben und wofür diese verwendet werden können. Zum Schluss werden noch Themen wie Sichtbarkeit, Benennung und Parameter erläutert.
...
Der Metatyp \enword{Inline Keyword} ist eine Kurzform des \enword{Compound Keywords}. Dabei kann man in einem \enword{Compound Keyword} ein neues \enword{Keyword} erstellen. Daher kommt auch der Name \enword{Inline Keyword}, weil es innerhalb eines anderen \enword{Keywords} erstellt wird. Im Beispiel \ref{prog:inlineKeyword} wird im \enword{Keyword} \enword{Anmelden an der PetClinic Anwendung} das \enword{Inline Keyword} \enword{Besitzer anlegen} definiert. In diesem \enword{Inline Keyowrd} werden alle Schritte zum Anlegen eines neuen Besitzers zusammengefasst. Ein \enword{Inline Keyword} muss nicht explizit aufgerufen werden. Die Definition eines \enword{Inline Keywords} ist auch automatisch auch der Aufruf dieses \enword{Keywords}. Der Anwendungsfall dieses Metatyps ist wiederum die Strukturierung, aber in diesem Fall innerhalb eines \enword{Keywords}. 
...
\end{JavaCode}
\caption{Rayden: Java-Implementierung des \enword{Print Keywords}}
...
\SuperPar
Die Verwendung eines \enword{Scripted Compound Keywords} sieht wie ein \enword{Inline Keyword} aus. Der große Unterschied ist, dass ein \enword{Inline Keyword} keine Parametersignatur im Gegensatz zu einem \enword{Scripted Compound Keyword} hat. Ein Beispiel für die Verwendung des \enword{IF Keywords} findet man im Codeausschnitt \ref{prog:ifKeywordUsage}.
...
\end{JavaCode}
\caption{\enword{ScriptEngineFactory} für Rayden registrieren}
